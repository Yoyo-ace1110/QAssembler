...
public:
    // constructor
    inline constexpr explicit Preprocessor(SrcFile& srcfile) noexcept 
    : file(srcfile) {}

    // function
    inline constexpr void replace_comments() noexcept {
    ...
    inline constexpr void standardize_EOL_and_space() noexcept {
        const size_t len = file.modified.size();
